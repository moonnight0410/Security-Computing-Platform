...
- 已审批安全输出包可以写入本域 `workspace/exports` 目录。
- 输出文件登记 `file_name`、`stored_path`、`sha256`、`byte_size`、生成时间和安全说明。
- 新增输出文件列表接口，便于本域留痕和人工归档。
- 新增审计链完整性校验接口，可识别审计记录内容篡改和链路断裂。
- 前端新增“写入本域文件”和“校验审计链完整性”操作。
- 本阶段仍禁止导出原始数据、加密数据、哈希数据、去标识摘要、派生键、对象级明细和低于阈值的聚合分组。
